...
@keyframes bounce2 { 0%, 100% { transform: translateY(0px) rotate(3deg); } 25% { transform: translateY(-15px) rotate(-3deg); } 50% { transform: translateY(-8px) rotate(2deg); } 75% { transform: translateY(-12px) rotate(-4deg); } }
...
.chars { display: flex; justify-content: center; align-items: center; gap: 1.2rem; margin-bottom: 0.8rem; }
.girl { font-size: 3.2rem; animation: bounce 1.4s ease-in-out infinite; display: inline-block; }
.heart { font-size: 2.2rem; animation: heartbeat 1.2s ease-in-out infinite; display: inline-block; }
.soldier { font-size: 3.2rem; animation: bounce2 1.6s ease-in-out infinite; display: inline-block; }
.title {
...
.and { font-family: 'Pacifico', cursive; font-size: 1.3rem; color: #C9A84C; margin: 0.1rem 0; }
.subtitle { font-size: 0.7rem; color: #B08FD4; letter-spacing: 2px; text-transform: uppercase; margin-top: 0.6rem; animation: shimmer 2s ease-in-out infinite; }
.stars { font-size: 1rem; letter-spacing: 0.3rem; margin: 0.4rem 0; animation: shimmer 2s ease-in-out infinite; }
...
.pin-title { font-family: 'Pacifico', cursive; font-size: 1.1rem; color: #C9A84C; margin-bottom: 0.2rem; }
.pin-hint { font-size: 0.7rem; color: rgba(176,143,212,0.6); margin-bottom: 1rem; font-style: italic; }
...
.pin-box:focus { border-color: #B08FD4; background: rgba(107,63,160,0.4); box-shadow: 0 0 8px rgba(176,143,212,0.3); transform: scale(1.05); }
...
.enter-btn:hover { background: linear-gradient(135deg, #8B5CC0, #5A7048); transform: translateY(-2px); box-shadow: 0 3px 12px rgba(107,63,160,0.4); }
.error-msg { color: #E24B4A; font-size: 0.75rem; margin-top: 0.6rem; display: none; }
...
<body>
    ...
    boxes.forEach((box, i) => {
        box.addEventListener('input', (e) => {
            if (box.value) { box.classList.add('filled'); box.classList.remove('error'); if (i < 5) boxes[i+1].focus(); }
        });
...
<div class="hero-section">
    ...
    <div style="margin-top:0.8rem; color:#C9A84C; font-family:'DM Sans',sans-serif; font-size:0.95rem; font-style:italic; font-weight:300;">
...
with tab1:
    ...
    with col_a:
        ...
        <div class="army-badge">
            ...
            <div style="font-size:1.5rem; font-family:'Playfair Display',serif; color:#C9A84C; font-weight:700;">{stats['days_since_army']}</div>
            <div style="font-size:0.75rem; letter-spacing:1px; text-transform:uppercase; color:#7A8C6A; margin-top:0.2rem;">Days in Service</div>
            <div style="font-size:0.8rem; margin-top:0.3rem; color:#E8EDE4;">Dawis joined Australian Army<br>20 April 2026</div>
        </div>""", unsafe_allow_html=True)
        ...
        if not milestones_df.empty:
            ...
            for i, row in milestones_df.iterrows():
                ...
                fig.add_trace(go.Scatter(
                    ...
                    textfont=dict(color='#F0E9FA', size=9),
                    hovertemplate=f"<b>{row['title']}</b><br>{row['description']}<extra></extra>",
                    showlegend=False
                ))
                fig.add_shape(type='line', x0=row['date'], x1=row['date'], y0=1.0, y1=y_pos, line=dict(color=color, width=1, dash='dot'))

            fig.add_shape(type='line', x0=milestones_df['date'].min(), x1=date.today(), y0=1, y1=1, line=dict(color='rgba(176,143,212,0.4)', width=2))
            fig.update_layout(
                title=dict(text='Our Journey Together', font=dict(color='#F0E9FA', size=13, family='Plus Jakarta Sans')),
                paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)',
                yaxis=dict(visible=False, range=[0.6, 1.5]), xaxis=dict(showgrid=False, color='#B08FD4'),
                height=220, margin=dict(l=10, r=10, t=30, b=10)
            )
            st.plotly_chart(fig, use_container_width=True)

    st.markdown('<div class="section-title" style="margin-top:0.8rem">Countdown to Anniversary 🎉</div>', unsafe_allow_html=True)
    progress = max(0, min(1, 1 - (stats['days_to_anniversary'] / 365)))
    st.progress(progress)
    st.markdown(f'<div style="color:#B08FD4; font-size:0.8rem; text-align:center;">{stats["days_to_anniversary"]} days until 22 August {stats["next_anniversary"].year}</div>', unsafe_allow_html=True)
...
with tab2:
    ...
    if memories_df.empty:
        st.markdown('<div style="text-align:center; color:#B08FD4; padding:2rem;">No memories yet — add your first one!</div>', unsafe_allow_html=True)
    else:
...
with tab3:
    ...
    for _, row in milestones_df.iterrows():
        all_events.append({'date': row['date'], 'title': row['title'], 'desc': row['description'], 'type': 'milestone', 'color': '#C9A84C'})
    for _, row in memories_df.iterrows():
        all_events.append({'date': row['date'], 'title': f"{row['emoji']} {row['title']}", 'desc': row['description'], 'type': row['category'], 'color': '#B08FD4'})

    all_events = sorted(all_events, key=lambda x: x['date'], reverse=True)
    for event in all_events:
        st.markdown(f"""
        <div style="border-left: 3px solid {event['color']}; padding: 0.5rem 0.8rem; margin-bottom: 0.6rem; background: rgba(61,26,110,0.3); border-radius: 0 8px 8px 0;">
            <div style="font-size:0.7rem; color:{event['color']}; text-transform:uppercase;">{event['date']} · {event['type']}</div>
            <div style="font-size:0.95rem; font-weight:600; color:#F0E9FA;">{event['title']}</div>
            <div style="font-size:0.8rem; color:#B08FD4;">{event['desc'] or ''}</div>
        </div>""", unsafe_allow_html=True)
...
with tab4:
    ...
    with col_form1:
        mem_title = st.text_input("Memory title *", placeholder="e.g. Queenstown trip")
        mem_desc = st.text_area("Description", placeholder="Tell the story...", height=80)
        mem_date = st.date_input("Date", value=date.today())
    with col_form2:
        mem_category = st.selectbox("Category", ["date night", "travel", "milestone", "everyday", "food", "special occasion", "army life", "other"])
